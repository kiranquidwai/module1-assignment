...
class SensorResource(resource.ObservableResource):
    ...
    async def _update_loop(self) -> None:
        while True:
            ...
            #log.info(f"{self.line}/{self.sensor_type} updated: {self._reading}" )
    ...
    async def render_get(self, request: Message) -> Message:
        ...
        for i in range(60):
            manifest["firmware"].append({
                ...
                "version": f"v1.{i}",
                "checksum": f"abc123xyz{i}",
                "update_url": f"https://updates.example.com/fw/{i}.bin",
                "line": random.choice(["line1", "line2"]),
                "type": random.choice(
